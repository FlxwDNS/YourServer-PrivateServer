...
public final class InstalledPluginInventory {
    ...
    public InstalledPluginInventory(Player player, Server server) {
        var gui = PagedGui.items()
                ...
                .setContent(this.getPlugins().stream().map(it -> new SimpleItem(new ItemBuilder(it.material()).setDisplayName("§a" + it.name()).setLegacyLore(List.of(
                        "§7",
                        "§7" + it.description(),
                        "§7",
                        "§7ID: §e" + (it.id().startsWith("https://") ? "Nicht vorhanden" : it.id()),
                        "§7",
                        "§eKlick §8» §cEntfernen§8."
                )), click -> {
